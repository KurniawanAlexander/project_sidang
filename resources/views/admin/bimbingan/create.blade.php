...
@section('main')
    ...
    <form action="{{ route('bimbingan.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="width-75">
            <div class="mb-3">
                ...
                <select class="form-select @error('pembimbing_id') is-invalid @enderror" name="pembimbing_id" required>
                    ...
                    <option value="{{ $tugasakhir->pembimbing1 }}"
                        {{ old('pembimbing_id') == $tugasakhir->pembimbing1 ? 'selected' : '' }}>
                        {{ $tugasakhir->pembimbingta1->nama_dosen }}</option>
                    <option value="{{ $tugasakhir->pembimbing2 }}"
                        {{ old('pembimbing_id') == $tugasakhir->pembimbing2 ? 'selected' : '' }}>
                        {{ $tugasakhir->pembimbingta2->nama_dosen }}</option>
                </select>
                @error('pembimbing_id')
            ...
            <div class="mb-3">
                ...
                <textarea id="pembahasan" name="pembahasan" class="form-control @error('pembahasan') is-invalid @enderror"
                    rows="5">{{ old('pembahasan', $data->pembahasan ?? '') }}</textarea>
                @error('pembahasan')
        ...
        <div class="mb-3 d-flex justify-content-start gap-2">
            ...
            <a href="/kartubimbingan/1" class="btn btn-outline-primary d-inline">
                Kembali
            </a>
        </div>
    </form>
