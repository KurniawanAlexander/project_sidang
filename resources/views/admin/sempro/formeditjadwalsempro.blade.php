...
@section('main')
    ...
    <form method="post" action="/editsemprojadwal/{{ $sempro->id }}" enctype="multipart/form-data">
        @csrf
        <div class="width-75">
            <div class="mb-3">
                ...
                <select name="penguji_id" class="form-select @error('penguji_id') is-invalid @enderror" required>
                    ...
                    @foreach ($dosen as $item)
                        <option value="{{ $item->id }}" @if (old('penguji_id', $sempro->penguji_id) == $item->id) selected @endif>
                            {{ $item->nama_dosen }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="datetime-local" class="form-control @error('tgl_sempro') is-invalid @enderror" id="tgl_sempro"
                    name="tgl_sempro"
                    value="{{ old('tgl_sempro', isset($sempro) ? \Carbon\Carbon::parse($sempro->tgl_sempro)->format('Y-m-d\TH:i') : '') }}"
                    required>
                @error('tgl_sempro')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="mb-3">
                ...
                <select name="ruangan_id" class="form-select @error('ruangan_id') is-invalid @enderror" required>
                    ...
                    @foreach ($ruangan as $item)
                        <option value="{{ $item->id }}" @if (old('ruangan_id', $sempro->ruangan_id) == $item->id) selected @endif>
                            {{ $item->ruangan }} {{ $item->kode_ruangan }}
                        </option>
                    @endforeach
                </select>
                @error('ruangan_id')
